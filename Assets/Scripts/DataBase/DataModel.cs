...
public class DataModel : MonoBehaviour
{
    ...
    public void WriteDataLocal(PlayerData dataNew)
    {
        playerData = dataNew;
        SaveData();
    }
    public void CheckDataLocalWithCloud(PlayerData dataCloud)
    {
        if (LoadData())
        {
            if (playerData.version_data >= dataCloud.version_data)
            {
                Debug.LogError("choose local data version: " + playerData.version_data);
            }
            else
            {
                playerData = dataCloud;
                Debug.LogError("choose local data version: " + playerData.version_data);
            }
        }
        else
